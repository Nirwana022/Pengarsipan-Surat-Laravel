                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="datatable table table-stripped">
                                        ...
                                        <tbody>
                                            @foreach($hasil as $mati)
                                                <tr>
                                                <td>{{ $no++ }}</td>
                                                <td>{{ $mati->nik }}</td>
                                                <td>{{ $mati->tgl_surat }}</td>
                                                <td>{{ $mati->status }}</td>
                                                <td>
                                                <a href="{{ url('suratmati/' . $mati->no_surat . '/print') }}" class="btn btn-success btn-sm">Print</a>
                                                <a href="{{ url('suratmati').'/'.$mati->no_surat.'/edit' }}" class="btn btn-primary btn-sm">Edit</a>
                                                <a href="{{ url('suratmati').'/'.$mati->no_surat.'/hapus' }}" class="btn btn-danger btn-sm" onclick="return confirm('Yakin Ingin Menghapus Data Ini?');">Hapus</a>
                                                </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
    ...
    <div id="modal-tambah" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ url('suratmati/create') }}" method="POST">
                    @csrf
